...
void processUserInputs(bool & running)
{
    ...
    while(SDL_PollEvent(&e)) 
    {
        ...
        if((e.key.keysym.sym == 'x' && e.type == SDL_KEYDOWN))
		{
			myCam.yaw -= (cos((myCam.yaw / 180.0) * M_PI)) * 0.5;
		}
		if((e.key.keysym.sym == 'z' && e.type == SDL_KEYDOWN))
		{
			myCam.yaw += (cos((myCam.yaw / 180.0) * M_PI)) * 0.5;
		}
		if((e.key.keysym.sym == 'f' && e.type == SDL_KEYDOWN))
		{
			myCam.pitch += (cos((myCam.yaw / 180.0) * M_PI)) * 0.5;
		}
